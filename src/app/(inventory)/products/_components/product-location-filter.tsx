...
export function ProductLocationFilter({ value, onChange }: ProductLocationFilterProps) {
  return (
    <div className="relative w-full sm:w-56">
      <label htmlFor="product-location-filter" className="sr-only">
        보관위치 필터
      </label>
      <MapPin
        aria-hidden="true"
      ...
      <select
        ...
        onChange={(event) => {
          if (isStorageFilter(event.target.value)) {
