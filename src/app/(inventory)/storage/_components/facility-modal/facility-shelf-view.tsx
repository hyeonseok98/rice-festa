...
interface FacilityShelfViewProps {
  ...
  feedback: { tone: 'success' | 'danger' | 'progress'; message: string } | null;
...
export function FacilityShelfView({ facility, placements, focusedProductId, draft, feedback, isDropSaving, onSelectProduct, onSelectSlot, onDropPlacement }: FacilityShelfViewProps) {
  ...
  return (
    <div className={`min-h-0 bg-[#eef1f4] p-3 md:p-5 ${needsScroll ? 'overflow-y-auto' : 'overflow-hidden'}`}>
      ...
      {feedback ? <p role={feedback.tone === 'danger' ? 'alert' : 'status'} className={`mb-3 rounded-lg px-3 py-2 text-xs font-semibold ${feedback.tone === 'danger' ? 'bg-danger-soft text-danger' : feedback.tone === 'success' ? 'bg-success-soft text-success' : 'bg-primary-soft text-primary'}`}>{feedback.message}</p> : null}
...
function FacilityLevelRow({ level, levelCount, placements, focusedProductId, draft, isDropSaving, onSelectProduct, onSelectSlot, onDropPlacement }: { level: StorageLevel; levelCount: number; placements: FacilityProductPlacement[]; focusedProductId: string | null; draft: PlacementDraft | null; isDropSaving: boolean; onSelectProduct: (productId: string) => void; onSelectSlot?: (levelNumber: number, slotNumber: number) => void; onDropPlacement: (item: FacilityPlacementDragItem, target: FacilityPlacementDropTarget) => void }) {
    ...
    <section className="relative flex min-h-18 flex-1 flex-col border-b-6 border-[#626d79] bg-surface px-2 pt-6 pb-1.5 md:min-h-22">
      ...
      {behind.length || draft?.isBehind ? (
        <div className="relative mb-1 min-h-8 rounded border border-dashed border-border-strong bg-[#e8edf2]">
          ...
          <SlotGrid level={level} placements={behind} focusedProductId={focusedProductId} draft={draft?.isBehind ? draft : null} isBehind isDropSaving={isDropSaving} onSelectProduct={onSelectProduct} onSelectSlot={onSelectSlot} onDropPlacement={onDropPlacement} compact />
        </div>
      ) : null}
      <SlotGrid level={level} placements={front} focusedProductId={focusedProductId} draft={draft && !draft.isBehind ? draft : null} isBehind={false} isDropSaving={isDropSaving} onSelectProduct={onSelectProduct} onSelectSlot={onSelectSlot} onDropPlacement={onDropPlacement} />
    </section>
  );
}

function SlotGrid({ level, placements, focusedProductId, draft, isBehind, isDropSaving, onSelectProduct, onSelectSlot, onDropPlacement, compact = false }: { level: StorageLevel; placements: FacilityProductPlacement[]; focusedProductId: string | null; draft: PlacementDraft | null; isBehind: boolean; isDropSaving: boolean; onSelectProduct: (productId: string) => void; onSelectSlot?: (levelNumber: number, slotNumber: number) => void; onDropPlacement: (item: FacilityPlacementDragItem, target: FacilityPlacementDropTarget) => void; compact?: boolean }) {
  ...
  return (
    <div className={`relative grid min-h-0 flex-1 ${compact ? 'h-8' : ''}`} style={{ gridTemplateColumns: `repeat(${level.slotCount}, minmax(0, 1fr))` }} onDragOver={dragOver} onDragLeave={(event) => { if (!event.currentTarget.contains(event.relatedTarget as Node | null)) setDropSlot(null); }} onDrop={dropPlacement}>
      ...
      <div className="pointer-events-none absolute inset-0 grid" style={{ gridTemplateColumns: `repeat(${level.slotCount}, minmax(0, 1fr))` }}>
        {placements.map(({ product, placement }) => <PlacementBlock key={`${product.id}-${placement.id}`} product={product} placement={placement} slotCount={level.slotCount} focused={focusedProductId === product.id} dragDisabled={isDropSaving} onSelect={() => onSelectProduct(product.id)} />)}
      </div>
    </div>
  );
}

function PlacementBlock({ product, placement, slotCount, focused, dragDisabled, onSelect }: { product: Product; placement: StoragePlacement; slotCount: number; focused: boolean; dragDisabled: boolean; onSelect: () => void }) {
